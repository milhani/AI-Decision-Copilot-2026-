...
function chatFree(ctx: MockAiContext, messages: AiMessage[]): MockAiResult {
  const lastUser = [...messages].reverse().find((m) => m.role === 'user')?.content ?? ''
  const projectName = ctx.project?.name ?? 'проект'
  const channels = ctx.project?.channels?.join(', ') || 'не указаны'
  const goal = ctx.project?.goal

  if (!ctx.hasData) {
    return {
      confidence: 'средняя',
      content: `По проекту **${projectName}** (каналы: ${channels})${goal ? `, цель: «${goal}»` : ''}.

${lastUser ? `По вашему вопросу: ${lastUser}\n\n` : ''}Метрик постов пока нет — импортируйте CSV в разделе «Импорт данных», и я смогу опираться на конкретные цифры. Пока могу обсуждать стратегию, гипотезы и подходы в контексте вашей ниши.

Если нужен разбор цифр — начните с импорта за последние 30–90 дней.`,
    }
  }

  const { avgEr, avgReach, postCount } = ctx.aggregated
  const hypotheses =
    ctx.activeHypotheses.length > 0
      ? ctx.activeHypotheses.map((h) => h.title).join(', ')
      : 'нет активных'

  return {
    confidence: 'средняя',
    content: `**${projectName}** — ${postCount} постов за период, ср. охват ${Math.round(avgReach)}, ср. ER ${avgEr.toFixed(2)}%. Каналы: ${channels}. Активные гипотезы: ${hypotheses}.

${lastUser ? `${lastUser}\n\n` : ''}С учётом данных проекта: если речь о метриках — смотрите на разброс между топ- и слабыми постами; если о стратегии — лучше оформить идею как гипотезу с KPI и сроком 14 дней.

Могу углубиться в конкретный аспект — метрики, гипотезы, контент-форматы или цели проекта.`,
  }
}
